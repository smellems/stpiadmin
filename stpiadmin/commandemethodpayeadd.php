<?php
	...
	$arrObjMethodPayeLg = array();
	foreach ($arrLang as $strLg)	
	{
		$arrObjMethodPayeLg[$strLg] = new clsmethodpayelg();
	...
	if ($ok)
	{
		...
		{
			if (!$nbMethodPayeID = $objMethodPaye->stpi_insert())
			{
				$ok = false;
			}
			if ($ok)
			{
				foreach ($arrLang as $strLg)	
				{
					if ($ok)
					{
						if ($arrObjMethodPayeLg[$strLg]->stpi_setNbMethodPayeID($nbMethodPayeID))
						{
							if (!$arrObjMethodPayeLg[$strLg]->stpi_insert())
							{
								$ok = false;
							}
						}
