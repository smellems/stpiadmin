...
class clstypeeventlg
{
	...
	public function stpi_chkNbID($nnbID)
	{
		...
		if (!$this->objBdd->stpi_chkExists($nnbID, "nbTypeEventLgID", "stpi_event_TypeEvent_Lg"))
		{
	...
	public function stpi_chkNbTypeEventID($nnbTypeEventID)
	{
		if (!$this->objBdd->stpi_chkInputToBdd($nnbTypeEventID))
		{
			print("<span style=\"color:#FF0000;\">" . $this->objTexte->stpi_getArrErrTxt("invalid") . "&nbsp;(lg)</span><br/>\n");
			return false;				
		}
		if (!$this->objBdd->stpi_chkExists($nnbTypeEventID, "nbTypeEventID", "stpi_event_TypeEvent"))
		{
			print("<span style=\"color:#FF0000;\">" . $this->objTexte->stpi_getArrErrTxt("noexists") . "&nbsp;(lg)</span><br/>\n");
			return false;
		}
	...
	public function stpi_setNbID($nnbID)
	{
		...
		$SQL = "SELECT nbTypeEventID, strName, strDesc, strLg FROM stpi_event_TypeEvent_Lg WHERE nbTypeEventLgID=" . $this->nbID;
	...
	{
		if (!$this->stpi_chkNbTypeEventID($nnbTypeEventID))
		{
			return false;				
		}
	...
	{
		$SQL = "INSERT INTO stpi_event_TypeEvent_Lg (nbTypeEventID, strName, strDesc, strLg)";
		$SQL .= " VALUES ('" . $this->objBdd->stpi_trsInputToBdd($this->nbTypeEventID) . "',";
	...
	{
		$SQL = "UPDATE stpi_event_TypeEvent_Lg";
		...
		$SQL .= " WHERE nbTypeEventLgID='" . $this->objBdd->stpi_trsInputToBdd($this->nbID) . "'";
	...
	public function stpi_deleteTypeEventId($nnbTypeEventID)
	{
		if (!$this->stpi_chkNbTypeEventID($nnbTypeEventID))
		{
			return false;				
		}
		$SQL = "DELETE FROM stpi_event_TypeEvent_Lg WHERE nbTypeEventID='" . $this->objBdd->stpi_trsInputToBdd($nnbTypeEventID) . "'";
	...
	public function stpi_selNbTypeEventID()
	{
		$this->objLang->stpi_setArrLang();
		$arrLang = $this->objLang->stpi_getArrLang();
		$arrID = array();
		...
		{
			$SQL = "SELECT nbTypeEventLgID";
			$SQL .= " FROM stpi_event_TypeEvent_Lg";
			$SQL .= " WHERE nbTypeEventID='" . $this->nbTypeEventID . "'";
			$SQL .= " AND strLg = '" . $strLg . "'";
			if ($result = $this->objBdd->stpi_select($SQL))
			{
				if ($row = mysql_fetch_assoc($result))
				{
					$arrID[$strLg] = $row["nbTypeEventLgID"];
				}
				else
				{
	...
	public function stpi_selNbTypeEventIDLG()
	{
		$SQL = "SELECT nbTypeEventLgID";
		$SQL .= " FROM stpi_event_TypeEvent_Lg";
		$SQL .= " WHERE nbTypeEventID='" . $this->nbTypeEventID . "'";
	...
	public function stpi_selSearchName($nstrName)
	{
		...
		$SQL = "SELECT DISTINCT nbTypeEventID";
		$SQL .= " FROM stpi_event_TypeEvent_Lg";
