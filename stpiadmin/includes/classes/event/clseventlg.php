...
class clseventlg
{
	...
	public function stpi_chkNbID($nnbID)
	{
		...
		if (!$this->objBdd->stpi_chkExists($nnbID, "nbEventLgID", "stpi_event_Event_Lg"))
		{
	...
	{
		if (!$this->objBdd->stpi_chkInputToBdd($nnbEventID))
		{
			print("<span style=\"color:#FF0000;\">" . $this->objTexte->stpi_getArrErrTxt("invalid") . "&nbsp;(lg-nbEventID)</span><br/>\n");
			return false;				
		}
		if (!$this->objBdd->stpi_chkExists($nnbEventID, "nbEventID", "stpi_event_Event"))
		{
			print("<span style=\"color:#FF0000;\">" . $this->objTexte->stpi_getArrErrTxt("noexists") . "&nbsp;(lg-nbEventID)</span><br/>\n");
			return false;
		}
		return true;
	}
	...
	{
		if (!$this->objBdd->stpi_chkInputToBdd($nstrDesc) AND $nstrDesc != "")
		{
	...
	{
		if (!$this->objBdd->stpi_chkInputToBdd($nstrLien) AND $nstrLien != "")
		{
	...
	public function stpi_setNbID($nnbID)
	{
		...
		$SQL = "SELECT nbEventID, strName, strDesc, strLien, strLg FROM stpi_event_Event_Lg WHERE nbEventLgID=" . $this->nbID;
	...
	{
		$SQL = "INSERT INTO stpi_event_Event_Lg (nbEventID, strName, strDesc, strLien, strLg)";
	...
	{
		$SQL = "UPDATE stpi_event_Event_Lg";
		...
		$SQL .= " WHERE nbEventLgID='" . $this->objBdd->stpi_trsInputToBdd($this->nbID) . "'";
	...
	public function stpi_deleteEventId($nnbEventID)
	{
		...
		$SQL = "DELETE FROM stpi_event_Event_Lg WHERE nbEventID='" . $this->objBdd->stpi_trsInputToBdd($nnbEventID) . "'";
	...
	public function stpi_selNbEventID()
	{
		...
		{
			$SQL = "SELECT nbEventLgID";
			$SQL .= " FROM stpi_event_Event_Lg";
			$SQL .= " WHERE nbEventID='" . $this->nbEventID . "'";
			$SQL .= " AND strLg = '" . $strLg . "'";
			if ($result = $this->objBdd->stpi_select($SQL))
			{
				if ($row = mysql_fetch_assoc($result))
				{
					$arrID[$strLg] = $row["nbEventLgID"];
				}
				else
				{
	...
	public function stpi_selNbEventIDLG()
	{
		$SQL = "SELECT nbEventLgID";
		$SQL .= " FROM stpi_event_Event_Lg";
		$SQL .= " WHERE nbEventID='" . $this->nbEventID . "'";
