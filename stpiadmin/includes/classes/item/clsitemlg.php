...
class clsitemlg
{
	...
	public function stpi_chkNbID($nnbID)
	{
		...
		if (!$this->objBdd->stpi_chkExists($nnbID, "nbItemLgID", "stpi_item_Item_Lg"))
		{
	...
	public function stpi_chkNbItemID($nnbItemID)
	{
		if (!$this->objBdd->stpi_chkInputToBdd($nnbItemID))
		{
			print("<span style=\"color:#FF0000;\">" . $this->objTexte->stpi_getArrErrTxt("invalid") . "&nbsp;(lg-nbItemID)</span><br/>\n");
			return false;				
		}
		if (!$this->objBdd->stpi_chkExists($nnbItemID, "nbItemID", "stpi_item_Item"))
		{
			print("<span style=\"color:#FF0000;\">" . $this->objTexte->stpi_getArrErrTxt("noexists") . "&nbsp;(lg-nbItemID)</span><br/>\n");
			return false;
		}
		return true;
	}
	...
	public function stpi_setNbID($nnbID)
	{
		...
		$SQL = "SELECT nbItemID, strName, strDesc, strLg FROM stpi_item_Item_Lg WHERE nbItemLgID=" . $this->nbID;
	...
	{
		$SQL = "INSERT INTO stpi_item_Item_Lg (nbItemID, strName, strDesc, strLg)";
	...
	public function stpi_update()
	{
		...
		$SQL .= " WHERE nbItemLgID='" . $this->objBdd->stpi_trsInputToBdd($this->nbID) . "'";
	...
	public function stpi_deleteItemId($nnbItemID)
	{
		if (!$this->stpi_chkNbItemID($nnbItemID))
		{
			return false;				
		}
		$SQL = "DELETE FROM stpi_item_Item_Lg WHERE nbItemID='" . $this->objBdd->stpi_trsInputToBdd($nnbItemID) . "'";
	...
	public function stpi_selNbItemID()
	{
		...
		{
			$SQL = "SELECT nbItemLgID";
			$SQL .= " FROM stpi_item_Item_Lg";
			$SQL .= " WHERE nbItemID='" . $this->nbItemID . "'";
			$SQL .= " AND strLg = '" . $strLg . "'";
			if ($result = $this->objBdd->stpi_select($SQL))
			{
				if ($row = mysql_fetch_assoc($result))
				{
					$arrID[$strLg] = $row["nbItemLgID"];
				}
				else
				{
					return false;
	...
	public function stpi_selNbItemIDLG()
	{
		$SQL = "SELECT nbItemLgID";
		$SQL .= " FROM stpi_item_Item_Lg";
		$SQL .= " WHERE nbItemID='" . $this->nbItemID . "'";
